...
class BuildingListTableWidget extends StatelessWidget {
  const BuildingListTableWidget({
    super.key,
    required this.buildingList,
    required BuildingBloc buildingBloc,
  ...
  Widget build(BuildContext context) {
    ...
    final ValueNotifier<List<Building>> sortedBuildingList =
        ValueNotifier(buildingList);
    final ValueNotifier<bool> sortAscending = ValueNotifier(true);

    final ValueNotifier<int?> sortedColumnIndex = ValueNotifier(null);

    void sort<T>(
        Comparable<T> Function(Building building) getField, int columnIndex) {
      ...
      sortedBuildingList.value = List.from(buildingList)
        ..sort((a, b) {
    ...
    return SingleChildScrollView(
      child: ValueListenableBuilder<List<Building>>(
        valueListenable: sortedBuildingList,
              ...
              columns: <DataColumn>[
                DataColumn(
                  ...
                  onSort: (columnIndex, _) {
                    sort<num>((building) => building.id, columnIndex);
                  },
                ),
                DataColumn(
                  label: Expanded(
                  ...
                  onSort: (columnIndex, _) {
                    sort<String>(
                        (building) => building.name.toString(), columnIndex);
                  },
                ),
                DataColumn(
                  label: Expanded(
                  ...
                  onSort: (columnIndex, _) {
                    sort<String>((building) => building.description.toString(),
                        columnIndex);
                  },
                ),
                DataColumn(
                                ...
                                onPressed: () async {
                                  final result = await showDialog<bool>(
                                    ...
                                    builder: (context) => AlertDialog(
                                      ...
                                      content: Text(
                                          'Вы уверены что хотите удалить объект строительства "${sortedList[i].name}" ?'),
                                      actions: [
                                        TextButton(
                                          onPressed: () =>
                                  ...
                                  _buildingBloc.add(DeleteBuildingEvent(
                                      id: sortedList[i].id));
                                },
                              ),
                            ],
