...
class TypeWorkApi implements ITypeWorkApi {
  final dio = Dio();
  ...
  Future<(bool, String, dynamic)> getTypeWorkShort(
    ...
    final response =
        await dio.get('/api/v1/types-of-work/?view_fields=["id","name"]');
  ...
  Future<(bool, String, dynamic)> getTypeWorkList({
    ...
    final response =
        await dio.get('/api/v1/types-of-work/', queryParameters: params);
    return (false, '', response.data);
  }
  ...
  Future<(bool, String, dynamic)> getTypeWorkDetail({
    ...
    dio.options.headers["Authorization"] = 'Token $token';
    final response = await dio.get('/api/v1/types-of-work/$id/');
    return (false, '', response.data);
  }
  ...
  Future<(bool, String, dynamic)> createTypeWork({
    ...
    try {
      dio.options.headers["Authorization"] = 'Token $token';
      final response = await dio.post('/api/v1/types-of-work/', data: {
        "name": name,
      ...
      return (response.statusCode == 201)
          ? (true, '', response.data)
          : (false, 'Failed to edit types-of-work', null);
    } catch (e) {
  ...
  Future<(bool, String, dynamic)> editTypeWork({
    ...
    try {
      dio.options.headers["Authorization"] = 'Token $token';
      final response = await dio.patch('/api/v1/types-of-work/$id/', data: {
        "name": name,
        "description": description,
      });
  ...
  Future<(bool, String, dynamic)> deleteTypeWork({
    ...
    dio.options.headers["Authorization"] = 'Token $token';
    final response = await dio.delete('/api/v1/types-of-work/$id/');
    return (false, '', response.data);
  }
}
